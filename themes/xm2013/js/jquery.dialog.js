	...
	init : function(){
		...
		if($.dialog){
			...
			$.dialog.addSkin('bluebox',{
				'div':'border:0px solid #BFBFBF;padding:8px;background-color:#8D8D8D;',
		...
		if (options.type == 'url'){
			...
			$.get(url, function(data, state){
				...
				setTimeout(function(){
					...
					dialog.animate({top:pos.top,left:pos.left,opacity:1}, 200);
				}, 100);
			});
			...
			self.resize(id);
			var pos = self.getPosition(id);
			dialog.css({top:pos.top,left:pos.left,opacity:1});
		...
		if(options.minScrollTop){
			...
			if(options.effects && typeof options['isLoad'] == 'undefined'){
				...
				switch(options.effects){
					case 'down':
						dialog.css({left:pos.left, top:-1000,display:'block',opacity:0.1}).animate({top:top,opacity:1},500);
					break;
					case 'up':
						dialog.css({left:pos.left, top:top+height,display:'block',opacity:0.1}).animate({top:top,opacity:1},500);
					break;
					case 'fade':
						//var duration = (typeof options.position == "object")?200:300;
						dialog.css({left:pos.left, top:pos.top,display:'block', opacity:0.1}).animate({opacity:1}, 200);
					break;
				}
			...
			options['clockScroll']=setTimeout(function(){
				...
				if(options.minScrollTop){
					if(self.scrollTop < options.minScrollTop){
						...
						dialog.animate({left:pos.left, top:pos.top, opacity:1}, 200);
					}
