...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
//首页推荐 by gaoyan
$is_index = isset($_REQUEST['is_show']) ? intval($_REQUEST['is_show']) : 0;
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order,is_index) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$_POST[sort_order]','$is_index')";
    $db->query($sql);
...
//-- 切换是否首页推荐 by gaoyan
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'toggle_indexshow')
{
    check_authz_json('brand_manage');
