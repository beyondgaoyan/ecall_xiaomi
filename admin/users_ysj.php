...
if ($_REQUEST['act'] == 'list')
{
    ...
    assign_query_info();
    $smarty->display('users_ysj_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('users_ysj_list.htm'), '', array('filter' => $user_list['filter'], 'page_count' => $user_list['page_count']));
}
...
//by gaoyan
    ...
    u.office_phone, u.home_phone, u.mobile_phone,u.isv,u.secondurl,u.status,u.statustime ".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
    ...
    if ($row)
    {
        ...
        //by gaoyan
        $user['isv']   = $row['isv'];
        $user['secondurl']   = $row['secondurl'];
        $user['status']   = $row['status'];
        $user['statustime']   = $row['statustime'];
    }
    else
    {
...
//        $user['formated_frozen_money'] = price_format(0);
    ...
    $smarty->display('user_ysj_info.htm');
}
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $users  =& init_users();
    $user_id = $_POST['id'];
     $userinfo = $users->get_profile_by_id($user_id);
    $status = empty($_POST['isok']) ? 0 : intval($_POST['isok']);
    $isv = empty($_POST['isv']) ? 0 : intval($_POST['isv']);
    $secondurl = empty($_POST['secondurl']) ? '' : trim($_POST['secondurl']);
	if($status==1){
		if(!isset($secondurl) || empty($secondurl)){
			$msg = '审核艺术家必须填写二级域名';
			sys_msg($msg, 1);
		}
		    /* 更新会员的其它信息 */
		    $other =  array();
		    $other['status'] = $status;
		    $other['statustime'] = $status ? time() :'';
		    $other['isv'] = $isv;
		    $other['secondurl'] = $status==1 ? $secondurl : '';
	} else {
		    /* 更新会员的其它信息 */
		    $other =  array();
		    $other['status'] = 0;
...
//by gaoyan  插入品牌艺术家
	if($status==1){
		 $sql = "SELECT brand_id FROM " . $ecs->table('brand') . " WHERE user_id =".$user_id;
        $col = $db->getCol($sql);
        ...
        if(empty($col) || !$col){
		    ...
		    $img_name ='';
		     /*处理URL*/
		    $site_url = sanitize_url($secondurl.'.trueart.com.cn');
		    /*插入数据*/
		    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order,user_id) ".
		           "VALUES ('$userinfo[user_name]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '100','$user_id')";
		    $db->query($sql);
		    admin_log($userinfo['user_name'],'add','brand');		
	    }
	}
    /* 记录管理员操作 */
    admin_log($username, 'edit', 'users_ysj');

    /* 提示信息 */
    $links[0]['text']    = $_LANG['goto_list'];
    $links[0]['href']    = 'users_ysj.php?act=list&' . list_link_postfix();
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 通过插件来删除艺术家 */
    $users =& init_users();
    $users->remove_user($username); //已经删除艺术家所有数据
...
function user_list()
{
    ...
    foreach ($extend_info_list AS $key => $val)
    {
        switch ($val['id'])
        {
            case 1:     $user_list[$i]['content'.$val[id]] = $user['msn']; break;
            case 2:     $user_list[$i]['content'.$val[id]] = $user['qq']; break;
            case 3:     $user_list[$i]['content'.$val[id]] = $user['office_phone']; break;
            case 4:     $user_list[$i]['content'.$val[id]] = $user['home_phone']; break;
            case 5:     $user_list[$i]['content'.$val[id]] = $user['mobile_phone']; break;
            default:    $user_list[$i]['content'.$val[id]] = empty($temp_arr[$val['id']]) ? '' : $temp_arr[$val['id']] ;
        }
    }
