...
if (!$smarty->is_cached('brandinfo.dwt', $cache_id))
{
...
//    $goodslist = brand_get_goods($brand_id, $cate, $size, $page, $sort, $order);
    ...
    $smarty->assign('brandinfo', brandinfo($brand_id));

    assign_pager('brandinfo',              $cate, $count, $size, $sort, $order, $page, '', $brand_id, 0, 0, $display); // 分页
    assign_dynamic('brandinfo'); // 动态内容
}

$smarty->display('brandinfo.dwt', $cache_id);
...
function brandinfo($brand_id){
	...
	$data =array(
			'brandimg'=>!empty($brandinfo[0]['brand_logo']) ? 'data/brandlogo/'.$brandinfo[0]['brand_logo']:'',//品牌图片全路径by gaoyan,
			'truename'=>$re[100],
			'brithday'=>$re[102],
			'jiguan'=>$re[101],
			'remark'=>$re[104],
